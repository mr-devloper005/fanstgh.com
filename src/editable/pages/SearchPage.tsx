      ...
      <main className="min-h-screen bg-[var(--editable-page-bg,#f7f7fb)] text-[var(--editable-page-text,#1b1530)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-[var(--editable-border)] bg-white/70 p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#6d28d9]">{pagesContent.search.hero.badge}</p>
              <h1 className="mt-5 text-4xl font-black leading-[0.98] tracking-[-0.06em] sm:text-5xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 opacity-70">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-[var(--editable-page-bg,#f7f7fb)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--editable-page-text,#1b1530)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#f7f7fb)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/listing" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black">Browse all listings <ArrowRight className="h-4 w-4" /></Link>
          </div>
